...
function resolveRequestError(error: unknown): string {
  if (error instanceof AccountApiError) {
    ...
    if (error.code === 'PHONE_ALREADY_VERIFIED') {
      return 'Телефон уже подключён';
    }
...
export default function PhoneLinkSheet({ visible, onClose }: PhoneLinkSheetProps) {
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      const response = await verifyPhoneLinkCode({
        challengeId,
        code: trimmedCode,
      });
        ...
        <KeyboardAvoidingView
          ...
          <View style={[styles.sheet, { paddingBottom: bottomInset }]}>
            <View style={styles.header}>
              <ThemedText style={styles.title}>Подключить телефон</ThemedText>
              ...
              {step === 'phone' ? (
                <>
                  <ThemedText themeColor="textSecondary" style={styles.subtitle}>
                    Телефон можно использовать для восстановления аккаунта.
                  </ThemedText>
